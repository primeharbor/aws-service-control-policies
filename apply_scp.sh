...
policy_id=`aws organizations list-policies --filter SERVICE_CONTROL_POLICY --output json | jq --arg p "$policy" -c '.Policies[] | select( .Name | contains("SecurityControls"))' | jq -r .Id `
